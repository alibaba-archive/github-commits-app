...
router.get('/themes', function * () {
  ...
  let data = yield request({
    method: 'GET',
    url: 'https://api.github.com/user/repos?access_token=' + tokenData.access_token,
    json: true,
    headers: headers
  }).then(function (body) {
...
router.get('/themes/:owner/:repo/commits', function * () {
  ...
  if (this.request.query.count && this.request.query.page) {
    url += `?page=${this.request.query.page}&count=${this.request.query.count}&access_token=${tokenData.access_token}`
  }
  let data = yield request({
    method: 'GET',
    url: url,
